__all__ = [
   'auto_gaussian_kernel_size', 'auto_gaussian_kernel_sigma',
   'check_gaussian_kernel_arg', 'get_mean_kernel', 'get_1d_gaussian',
   'get_2d_gaussian_kernel', 'mean_blur', 'gaussian_blur', 'bilateral_blur',
   'median_blur', 'maximum_blur', 'minimum_blur', 'geometric_mean_blur',
   'reciprocal_8UC1', 'reciprocal_8UC3', 'harmonic_mean_blur',
   'contraharmonic_mean_blur', 'midpoint_filter', 'alpha_trimmed_mean_blur', 'adaptive_mean_blur', 'adaptive_median_filter',
   'get_gradient_operator_names', 'get_gradient_operator',
   'get_line_operator_name', 'get_line_operator', 'gradient_norm', 
   'gradient_uint8_overflow', 'get_derivatives_of_gaussian',
   'get_difference_gaussian', 'get_laplacian_gaussian', 'laplacian',
   'laplacian_all', 'laplacian2', 'laplacian4', 'sobel', 'kirsch',
   'gaussian_gradient', 'marr_hildreth', 'unsharp_masking'
]
...
from numba import (
    njit, prange, typeof, int64, int32, int16, int8, uint64, uint32, uint16,
    uint8
)
...
from commons import is_iterable, pad_reflect101
from stats import quick_sort, mean_seq, max_min, median
from img_type import (
    IMG_ARRAY, ARR_8U2D, ARR_32F2D, ARR_32F1D, IMG_32F, KER_SIZE, IMG_8U
)
...
@boundscheck(False)
@wraparound(False)
def __valid_convolution_ksize(ksize: Union[int, KER_SIZE] = 3) -> KER_SIZE:
...
def check_gaussian_kernel_arg(
        ksize: Union[int, KER_SIZE], sigma_y: float, sigma_x: float
...
## Blurring
# Kernel getters
__SIGNATURE_GET_MEAN_KERNEL = [
    "float32[:,:](UniTuple(uint32,2))"
]
@njit(
    __SIGNATURE_GET_MEAN_KERNEL,
    nogil=True, cache=True, fastmath=True)
@wraparound(False)
def get_mean_kernel(ksize) -> ARR_32F2D:
...
@wraparound(False)
def get_1d_gaussian(ksize: int, sigma: float) -> ARR_32F1D:
...
@wraparound(False)
def mean_blur(
        img: IMG_ARRAY, ksize: Union[int, KER_SIZE] = (3, 3)
    ) -> IMG_ARRAY:
...
@wraparound(False)
def gaussian_blur(
        img: IMG_ARRAY, ksize: Union[int, KER_SIZE] = 3,
        sigma_y: float = 0, sigma_x: float = 0
    ) -> IMG_ARRAY:
    ...
    output : IMG_ARRAY
        Output image.
    """
    ksize, sigma_y, sigma_x = check_gaussian_kernel_arg(
        ksize, sigma_y, sigma_x)
...
@wraparound(False)
def bilateral_blur(
        img: IMG_ARRAY, ksize: int,
        sigma_color: float = 50, sigma_space: float = 50
    ) -> IMG_ARRAY:
...
def median_blur(img: IMG_ARRAY, ksize: int) -> IMG_ARRAY:
    ...
    if not ksize % 2 or ksize < 1:
        raise ValueError(
            "ksize must be a positive odd number.")
    if ksize == 1:
        return img
    else:
...
@boundscheck(False)
@wraparound(False)
def maximum_blur(img: IMG_ARRAY, ksize: Union[int, KER_SIZE]) -> IMG_ARRAY:
    ...
    if not ksize % 2 or ksize <= 1:
        raise TypeError(
            "ksize must be a positive odd number that greater than 1.")
    ksize = __valid_convolution_ksize(ksize)
...
@boundscheck(False)
@wraparound(False)
def minimum_blur(img: IMG_ARRAY, ksize: Union[int, KER_SIZE]) -> IMG_ARRAY:
...
@njit("float32[:,:](uint8[:,:])",
      nogil=True, cache=True, fastmath=True)
@wraparound(False)
def logarithm_8UC1(img: ARR_8U2D):
...
@njit("float32[:,:,:](uint8[:,:,:])",
      nogil=True, cache=True, fastmath=True)
@wraparound(False)
def logarithm_8UC3(img: ARR_8U2D):
    output = np.empty_like(img, dtype=np.float32)
    table = np.empty(256, dtype=np.float32)
    for i in range(256):
        table[i] = np.log(i)
...
@boundscheck(False)
@wraparound(False)
def geometric_mean_blur(
        img: IMG_8U, ksize: Union[int, KER_SIZE] = (3, 3)
    ) -> IMG_8U:
...
@njit("float32[:,:](uint8[:,:])",
      nogil=True, cache=True, fastmath=True)
@wraparound(False)
def reciprocal_8UC1(img: ARR_8U2D):
    output = np.empty_like(img, dtype=np.float32)
    table = np.empty(256, dtype=np.float32)
    for i in range(256):
...
@njit("float32[:,:,:](uint8[:,:,:])",
      nogil=True, cache=True, fastmath=True)
@wraparound(False)
def reciprocal_8UC3(img: ARR_8U2D):
    output = np.empty_like(img, dtype=np.float32)
    table = np.empty(256, dtype=np.float32)
    for i in range(256):
        table[i] = np.divide(1, i)
...
def __alpha_trimmed_mean(pad_img, originalSize, ksize, alpha):
    ...
    for y in prange(originalSize[0]):
        for x in prange(originalSize[1]):
            sort_ = quick_sort(
                pad_img[y:y+ksize[0],x:x+ksize[1]].flatten()
...
def alpha_trimmed_mean_blur(
        img: IMG_8U, ksize: Union[int, KER_SIZE] = (3, 3), alpha: int = 2
    ) -> IMG_8U:
...
@njit(
    __SIGNATURE_ADAPTIVE_NOISE_REDUCTION,
    nogil=True, cache=True, fastmath=True, parallel=False)
@wraparound(False)
    ...
    for y, row_var in enumerate(local_var):
        for x, var_ in enumerate(row_var):
            if (var_ < var_eta):
                ...
                output[y,x] = (
                    img[y,x] + np.multiply(c, trans[y,x])
                )
    return output
...
def adaptive_mean_blur(
        img: IMG_8U, ksize: Union[int, KER_SIZE] = (3, 3), var_eta: int = 1
    ) -> IMG_8U:
    ...
    # Adaptive decision
    blurred = __adaptive_mean(img, mean_blurred, translation,
                              local_var, var_eta)
    return cv2.convertScaleAbs(blurred)
...
def adaptive_median_filter(
        img: IMG_8U, kinitial: Union[int, KER_SIZE] = (3, 3),
        kfinal: Union[int, KER_SIZE] = (7,7)
    ) -> IMG_8U:
...
def get_gradient_operator_names():
    ...
    """
    return (
        "laplacians", "laplacians_all", "laplacian2", "laplacian4",
        "sobel", "prewitt", "robinson", "roberts", "kirsch", "scharr",
        "LoG"
    )
...
@boundscheck(False)
@wraparound(False)
def get_gradient_operator(grad_name: str) -> list[ARR_32F2D]:
    """Get a list of availible gradient operator name.
...
@boundscheck(False)
@wraparound(False)
def get_line_operator(op_name: str) -> ARR_32F2D:
    """Get a list of availible line-detection operator name.
...
def gradient_norm(
    ...
    if ntype == 2:
        return np.sqrt(
            np.sum(
                np.power(edges, 2, dtype=np.float32),
...
def gradient_uint8_overflow(
        norm: IMG_32F, otype: Literal[0, 1] = 0,
        percent: Union[int, float] = 1,
...
@wraparound(False)
def get_difference_gaussian(
        ksize: KER_SIZE, sigma1: float, sigma2: float
    ) -> ARR_32F2D:
    ...
    for y in range(ksize[0]):
        ...
        for x in range(ksize[1]):
            ...
            first = (
                consts1[0] * np.exp(dist*consts1[1])
            )
            kernel[y,x] = (
                first
                - consts2[0] * np.exp(dist*consts2[1])
            )
...
@njit([
    "float32[:,:](UniTuple(uint16,2),float32)"
], nogil=True, cache=True, fastmath=True)
@wraparound(False)
...
def laplacian(
        ...
        norm_type: Optional[Literal[0, 1, 2, 3]] = 2
    ) -> Union[IMG_32F, list[IMG_32F]]:
...
def laplacian2(
        img: IMG_ARRAY,
        threshold: Union[int,float] = 0,
        norm_type: Optional[Literal[0]] = 0
    ) -> IMG_32F:
...
def gaussian_gradient(
        img: IMG_ARRAY, ksize: Union[int, KER_SIZE] = 3,
        sigma_y: float = 1, sigma_x : float = 1,
        threshold: Union[int,float] = 0,
        norm_type: Optional[Literal[0, 1, 2, 3]] = 2
    ) -> Union[IMG_32F, list[IMG_32F]]:
    ...
    edges = np.array((
        cv2.filter2D(img, CV_32F, kernel_y),
        cv2.filter2D(img, CV_32F, kernel_x)
    ), np.float32)
    if norm_type is not None:
        output = gradient_norm(edges, norm_type)
...
def marr_hildreth(
        img: IMG_ARRAY, ksize: Union[int, KER_SIZE] = 3,
        sigma: float = 0, norm_type: Literal[0, 1, 2, 3] = 2,
        dtype: np.dtype = np.float32
    ):
...
@wraparound(False)
def unsharp_masking(
        img: IMG_ARRAY, ksize: Union[int, KER_SIZE] = (5, 5),
    ...
    if blurring == "mean":
        ...
        ksize, sigma_y, sigma_x = check_gaussian_kernel_arg(ksize,
                                                            sigma_y, sigma_x)
        kernel = get_2d_gaussian_kernel(ksize, sigma_y, sigma_x)
...
# def sharpening(img: IMG_ARRAY, grad: Union[str, KER_2D]):
