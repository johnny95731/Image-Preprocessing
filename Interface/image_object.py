...
from typing import TypeVar, Generic, Tuple, TypeVar, NewType, Union, Optional
...
import enhance
from noises import (
    add_uniform_noise, add_gaussian_noise, add_rayleigh_noise,
    add_gamma_noise, add_exponential_noise, add_single_color_noise,
    add_salt_and_pepper_noise, add_beta_noise
)
from seg import (
    thresholding, get_auto_thresholding_names, auto_threshold
)
from spatial_filters import (
    get_gradient_operator_names, get_gradient_operator, gradient_norm,
    gradient_uint8_overflow, mean_blur, gaussian_blur, median_blur,
...
SKEW = NewType("skew", np.float32)
KURTOSIS = NewType("kurtosis", np.float32)
MAX = NewType("max_", np.uint8)
MIN = NewType("min_", np.uint8)
...
class BasicOperators():
    ...
    def get_statistics(
            img: ARR_8U2D
        ) -> Tuple[MEAN, VAR, STD, SKEW, KURTOSIS, MAX, MIN, MEDIAN]:
        # Order statistice
...
class EnhanceOperators():
    ...
    def get_intensity_transform_names():
        return (
            "Linear Transformation", "Gamma Correction", "Log Transformation",
            "Arctan Transformation", "Logistic Correction", "Beta Correction",
            "Level Slicing Type I", "Level Slicing Type II",
            "Level Slicing Type II (Inv.)"
        )
    
    @staticmethod
    def intensity_transform(
            img: ARR_8U2D, op: int, args: list[float]
        ) -> ARR_8U2D:
        if op == 0: # Linear Transformation
            ...
            args = [
                (round(args[0]), round(args[1])), round(args[2]), round(args[3])
            ]
            return enhance.intensity_level_slicing_type1(img, *args)
        elif op == 7: # Level Slicing Type II
            args = [
                (round(args[0]), round(args[1])), round(args[2])
            ]
...
class SpatialOperators():
    ...
    def blurring(
            img: IMG_8U, op: str,
            ksize: Union[int, KER_SIZE], args: list[float]
        ) -> IMG_8U:
        if op == "mean":
            blurred = mean_blur(img, ksize,)
        elif op == "gaussian":
    ...
    @staticmethod
    def unsharp_masking(
            img: IMG_8U, op: str, amount: float,
            ksize: Union[int, KER_SIZE], args: list[float]
        ) -> IMG_8U:
        ...
        mask = cv2.addWeighted(
            img, 1+amount, blurred, -amount, 0, dtype=cv2.CV_32F
        )
    ...
    @staticmethod
    def gradient(
            img: IMG_8U, op: str, bordertype: int, norm_type: int,
            to_grayscale: bool, overflow_type: int, overflow_val: float,
            threshold_val: int
        ) -> IMG_8U:
        ...
        # Thresholding
        if (threshold_val > 0):
            output = cv2.threshold(output, threshold_val, 255,
                                   cv2.THRESH_TOZERO)[1]
        return output
    ...
    @staticmethod
    def canny(
            img: IMG_8U, ksize: int, sigma: float,
            threshold_val: int, threshold2_val: int, l2_gradient: bool
        ) -> IMG_8U:
        # -Gaussian Blur
        if ksize > 1:
        ...
        # -Canny
        return cv2.Canny(
            img, threshold_val, threshold2_val, apertureSize=3,
...
class NoiseOperators():
    ...
    def get_noises_names() -> tuple[str]:
        return (
            "uniform", "gaussian", "rayleigh", "gamma", "exponential", "beta",
            "salt-and-pepper",
            # "single color"
        )
        
    def add_noise(
            img: ARR_8U2D, noise: str, args: list[float]
        ) -> ARR_8U2D:
        if noise == "uniform":
...
class MorpologyOperators():
    ...
    def get_operaions_names():
            return (
            # OpenCV
            "Dilation", "Erosion", "Opening", "Closing", "Gradient", "Top Hat",
            "Black Hat", "Hit-or-Miss",
            # 1 SE
            "Pruning", "Connect Components", "Thinning", "Thickening",
            "Boundary Extraction", "Region Filling", "Border Clean", "Skelton",
            # 2 SE
            "Opening by Rec.", "Closing by Rec."
        )
    
    @staticmethod
    def basic_operation(
            img: IMG_8U, op: int, SE: np.ndarray, iter_: int, bordertype: int,
            SE2: Optional[np.ndarray]
        ) -> IMG_8U:
        
        if op < 8:
            return cv2.morphologyEx(
                img, op, SE, iterations=iter_,
                borderType=bordertype
            )
        elif op == 8:
...
class Image(Generic[IMG], BasicOperators):
    ...
    support_database = (
        "astronaut", "brick", "camera", "cat", "cell", "checkerboard",
        "clock", "coffee", "coins", "colorwheel", "grass",
        "gravel", "horse", "hubble_deep_field", "immunohistochemistry",
        "logo", "microaneurysms", "moon", "page", "retina", "rocket",
    ...
    support_database2 = (
        "brain", # (10, 256, 256)
        "kidney", # (16, 512, 512, 3)
        "lfw_subset", # (200, 25, 25)
        "nickel_solidification", # (11, 384, 512)
        "protein_transport", # (15, 2, 180, 183)
        "stereo_motorcycle", # (3, 500, 741, 3)
    )
    
    history_limit: int = 20 # Limit of history length of Image object
    
    def __init__(
            self, img: IMG_8U, name: str, file_extension: str, path: str):
    ...
    def read_image(path: str) -> IMG:
        img = cv2.imdecode(
            # cv2 does not support chinnese in filename/path
    ...
    def image(self, new_img: IMG_8U):
        if not isinstance(new_img, np.ndarray):
            try:
                ...
                print(
                    "img should be type NDArray[np.uint8],",
                    f"not {type(new_img)}"
                )
                raise ValueError
