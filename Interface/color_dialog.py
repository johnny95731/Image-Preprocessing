__all__ = [
    "FuncTransDialog", "PiecewiseLinearDialog", "SlicingDialog",
    "kMeansDialog", "HistogramMatchingDialog", "CLAHEDialog"
]
...
from commons import transform
import enhance
from colors import color_slicing
from seg import kmeans_seg
from image_object import EnhanceOperators
from img_type import ARR_8U2D
...
class FuncTransDialog(BasicDialog):
    ...
    def __create_info_region(self):   
        ...
        self.widget["canvas"], self.widget["plot"] = \
            MplCanvas(region, (1.125,2), visible=True).widget
        ...
        for i, c in enumerate(self.channels):
            ...
            for j in range(4):
                ...
                if self.is_color:
                    ...
                    layout_vals.addWidget(self.widget[f"arg{c}{j}_label"],
                                        2*j+1, i)
                    layout_vals.addWidget(self.spin_box[f"arg{c}{j}"],
                                        2*j+2, i)
                else:
    ...
    def main_work(self):
        ...
        for i, c in enumerate(self.channels):
            ...
            plot_y = EnhanceOperators.intensity_transform(
                plot_x, self.func, args)[0]
            self.widget["plot"].plot(
                plot_x[0], plot_y, color=c[0].lower(), alpha=0.7)
    ...
    def update_args_label(self):
        ...
        self.widget["formula"].setPixmap(
            MplCanvas.latex(self.func_info["formula"], 12))
        
    def func_changed(self, func: Optional[int] = None):
        self.func = func
...
class PiecewiseLinearDialog(BasicDialog):
    ...
    def __create_info_region(self):   
        ...
        self.widget["canvas"], self.widget["axis"] = \
            MplCanvas(region, (1.125,2), visible=True).widget
        layout_region.addWidget(self.widget["canvas"], 0, 0, 1, 4)
        n = np.arange(256)
    ...
    def cell_changed(self, row, col):
        ...
        text = item.text()
        if (not text or
            not text.replace('.', '', 1).isdigit() # Not a numerical text.
...
class SlicingDialog(BasicDialog):
    ...
    def __create_args_region(self):
        ...
        if self.is_color:
            ...
            self.spin_box[f"arg{i}-{j}"].valueChanged.connect(
                self.update_view)
            ...
            for i, _ in enumerate(self.channels):
                ...
                # Horizontal: Arguments of R, G, B, respectively.
                # Vertical: The 1st, 2nd, 3rd argument.
                layout_vals.addWidget(self.spin_box[f"arg{i}-{j}"],
                                    2*j+1, i)
            # radius
            j = 2
            i = 0
    ...
    # Functions
    def main_work(self):
        bg = int(self.spin_box[f"arg0-0"].value())
        radius = self.spin_box[f"arg0-2"].value()
        if self.is_color:
            center = [
                self.spin_box[f"arg{i}-1"].value()
                for i, _ in enumerate(self.channels)
            ]
            return color_slicing(
                self.original_image, center, radius, bg,
                self.combo_box["arg"].currentIndex()
            )
        else:
            center = self.spin_box[f"arg0-1"].value()
...
class kMeansDialog(BasicDialog):
    ...
    def main_work(self):
        return kmeans_seg(
            self.original_image, self.spin_box["arg"].value(),
            self.spin_box["max_iter"].value(), self.spin_box["tol"].value())
...
class HistogramMatchingDialog(BasicDialog):
    ...
    support_hist = (
        "exp(-(s-127.5)**2/1000)", "exp(-(s-127.5)**2/5000)",
        "s**2", "sqrt(s)", "1/sqrt(1+s)"
    )
    ...
    def __init__(self, parent: QWidget):
        ...
        if self.is_color:
            self.h, self.s, self.v = (
                cv2.split(cv2.cvtColor(self.original_image, cv2.COLOR_RGB2HSV))
            )
        # Main region
        self.__create_info_region()
        self.__create_args_region()
    ...
    def __create_info_region(self):   
        ...
        self.widget["canvas"], self.widget["plot"] = \
            MplCanvas(region, (1.125,2)).widget
        layout_region.addWidget(self.widget["canvas"], 0, 0, 1, 4)
    ...
    def __create_args_region(self):
        ...
        # -Combo box.
        hist_list = (
            "customization", *HistogramMatchingDialog.support_hist
        )
        self.combo_box["arg"] = QtWidgets.QComboBox()
        layout_options.addWidget(self.combo_box["arg"], 2)
        self.combo_box["arg"].addItems(hist_list)
    ...
    def main_work(self):
        ...
        try:
            ...
            print(e)
            edit_pos = self.widget["edit"].mapToGlobal(
                    self.widget["edit"].pos())
            edit_size = QPoint(
                0, self.widget["edit"].size().height())
            QToolTip.showText(
                edit_pos - edit_size, 
                "".join(
                    f"\"{formula}\" can't be calculate."
                )
            )
            return None
        pdf /= np.sum(pdf)
        # Plot pdf and cdf
        self.widget["plot"].bar(
            support, pdf, color="r", alpha=1
        )
        self.widget["canvas"].draw()
        #
        if len(self.channels) == 1:
    ...
    def func_changed(self, func: Optional[int] = None):
        if func:
            self.widget["edit"].setText(
                HistogramMatchingDialog.support_hist[func-1]
            )
        else:
...
class CLAHEDialog(BasicDialog):
    ...
    def __init__(self, parent: QWidget):
        ...
        if self.is_color:
            self.h, self.s, self.original_image = (
                cv2.split(cv2.cvtColor(self.original_image, cv2.COLOR_RGB2HSV))
            )
        # Main region
        self.__create_args_region()
