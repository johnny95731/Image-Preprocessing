...
from img_type import IMG_8U, ARR_8U2D, ARR_2D
...
@wraparound(False)
def region_filling(
        img: ARR_8U2D, marker: ARR_8U2D,
        kernel: ARR_2D = DEFAULT_KER, max_iter: int = 10
    ):
    """
...
def connected_component(
        img: ARR_8U2D, marker: ARR_8U2D,
        kernel: ARR_2D = DEFAULT_KER, max_iter: int = 20
    ) -> ARR_8U2D:
    """
...
def connected_component_region(
    ...
    indices = (
        indices[0][::-1]-1, indices[1][::-1]+1 # column-row
    )
...
def all_connected_components(
        img: ARR_8U2D, kernel: ARR_2D = DEFAULT_KER, max_iter: int = 100
    ):
    """
    ...
    while index != (-1, -1):
        ...
        components.append(
            connected_component(remain, marker, kernel, max_iter)
        )
...
def draw_connect_components(
        img: ARR_8U2D, kernel: ARR_2D = DEFAULT_KER, max_iter: int = 100,
        overlap: bool = True, copy: bool = True
    ):
    """
    ...
    if not overlap:
        ...
        for i, reg in enumerate(regions, 1):
            for j, reg2 in enumerate(regions[i:], i):
                ...
                if (reg[0][0]>=reg2[0][0] and reg[1][0]<=reg2[1][0] and # x
                    reg[0][1]>=reg2[0][1] and reg[1][1]<=reg2[1][1] # y
                    ):
                    remove_indices.append(i-1)
                elif ( # reg2 contains in reg
                    reg2[0][0]>=reg[0][0] and reg2[1][0]<=reg[1][0] and # x
                    reg2[0][1]>=reg[0][1] and reg2[1][1]<=reg[1][1] # y
                    ):
...
@boundscheck(False)
@wraparound(False)
def thinning(img: ARR_8U2D, max_iter: int = 1):
    # 細線化
    kernel1 = np.array(
        [[-1,-1,-1],
         [ 0, 1, 0],
    ...
    for _ in range(4*max_iter):
        temp = cv2.absdiff(
            output,
            cv2.morphologyEx(output, cv2.MORPH_HITMISS, kernel1)
        )
        output = cv2.absdiff(
            temp,
            cv2.morphologyEx(temp, cv2.MORPH_HITMISS, kernel2)
        )
...
@boundscheck(False)
@wraparound(False)
def thickening(img: ARR_8U2D, max_iter: int = 1):
    # 厚化
    # Equivalent:
    # bitwise_not(
    ...
    for _ in range(4*max_iter):
        ...
        output = cv2.absdiff(
            output,
            cv2.morphologyEx(output, cv2.MORPH_HITMISS, kernel1, borderValue=0)
        )
        output = cv2.absdiff(
            output,
            cv2.morphologyEx(output, cv2.MORPH_HITMISS, kernel2, borderValue=0)
        )
        kernel1 = cv2.rotate(kernel1, cv2.ROTATE_90_CLOCKWISE)
        kernel2 = cv2.rotate(kernel2, cv2.ROTATE_90_CLOCKWISE)
    return output
...
@boundscheck(False)
@wraparound(False)
def skelton(img: ARR_8U2D, kernel: ARR_8U2D) -> tuple[ARR_8U2D,int]:
    # 骨架
    ...
    while cv2.countNonZero(temp):
        ...
        output = cv2.bitwise_or(
            output, cv2.subtract(temp, opened)
        )
...
@boundscheck(False)
@wraparound(False)
def pruning(img: ARR_8U2D, thinning_iter: int = 5) -> ARR_8U2D:
    # 剪除
    ...
    for _ in range(thinning_iter):
        for _ in range(4):
            thined = cv2.absdiff(
                thined,
                cv2.morphologyEx(thined, cv2.MORPH_HITMISS, kernel1)
            )
            kernel1 = cv2.rotate(kernel1, cv2.ROTATE_90_CLOCKWISE)
        for _ in range(4):
            thined = cv2.absdiff(
                thined,
                cv2.morphologyEx(thined, cv2.MORPH_HITMISS, kernel2)
            )
            kernel2 = cv2.rotate(kernel2, cv2.ROTATE_90_CLOCKWISE)
    # step2 = find endpoint
...
def geodesic_dilation(
        img: ARR_8U2D, mask: ARR_8U2D, kernel: ARR_8U2D = DEFAULT_KER,
        iterations: int = 1
    ) -> ARR_8U2D:
    """
...
@boundscheck(False)
@wraparound(False)
def opening_by_rec(img, ker_erosion=DEFAULT_KER, ker_rec=DEFAULT_KER,
                   order: int = 1):
    """
...
@boundscheck(False)
@wraparound(False)
def closing_by_rec(img, ker_erosion=DEFAULT_KER, ker_rec=DEFAULT_KER,
                   order: int = 1):
    """
...
def auto_region_filling(img, kernel=DEFAULT_KER):
    ...
    return cv2.bitwise_not(
        geodesic_dilation(marker, neg, kernel, iterations=None)
    )
