__all__ = [
    "linear_transformation", "linear_transformation_8UC1",
    "piecewise_linear_function", "piecewise_linear_transformation",
    "intensity_level_slicing_type1", "intensity_level_slicing_type2",
    "intensity_level_slicing_type2_inv", "gamma_correction",
    "gamma_correction_8UC1", "gamma_correction_32FC1", "log_transformation",
    "log_transformation_8UC1", "arctan_transformation_8UC1",
    "logistic_correction_8UC1", "beta_correction", "beta_correction_8UC1",
    "auto_gamma_correction_PB_PZ", "histogram_equalization",
    "histogram_matching"
]
...
import commons
import stats
from img_type import ARR_8U2D, ARR_32F2D, ARR_8U1D, ARR_1D
...
def __clip_and_transform_dtype(
        img: NDArray, clip_: bool, dtype: np.dtype, maximum: float = 255
    ) -> NDArray:
...
def linear_transformation(
        img: NDArray, ratio: float = 1, bias: float = 0,
        maximum: Optional[float] = None, clip_: bool = True,
...
def linear_transformation_8UC1(
        img: ARR_8U2D, ratio: float = 1, bias: float = 0
    ) -> ARR_8U2D:
    """Intensity transformation by linear transformation:
        T(img) = ratio*img + bias.
    ...
    for i in range(256):
        ...
        if val < 0:
            ...
            table[i] = round(val)
    return commons.transform(img, table)
...
def piecewise_linear_function(
        x_points: ARR_1D, y_heights: ARR_1D
    ) -> ARR_8U1D:
    ...
    if x_points[0] > 0:
        slope = (y_heights[1]-y_heights[0]) \
                / (x_points[1]-x_points[0])
        output[0] = round(
            slope*(0-x_points[0]) + y_heights[0]
        )
    if x_points[-1] == 255:
        output[-1] = round(y_heights[-1])
    elif x_points[-1] < 255:
        slope = (y_heights[-1]-y_heights[-2]) \
                / (x_points[-1]-x_points[-2])
        output[-1] = round(
            slope*(255-x_points[-1]) + y_heights[-1]
        )
    return output
...
def piecewise_linear_transformation(
        img: ARR_8U2D, x_points: ARR_1D, y_heights: ARR_1D
    ) -> ARR_8U2D:
...
def intensity_level_slicing_type1(
        img: ARR_8U2D, region: Tuple[int, int], fg: int, bg: int
    ) -> ARR_8U2D:
...
def intensity_level_slicing_type2(
        img: ARR_8U2D, region: Tuple[int, int], level: int
    ) -> ARR_8U2D:
...
def gamma_correction(
        img: NDArray, gamma: Union[float,NDArray] = 1,
        ratio: Optional[float] = None, bias: float = 0,
        maximum: Optional[float] = None,
        clip_: Optional[bool] = None, dtype: np.dtype = np.uint8
    ) -> NDArray:
...
def gamma_correction_8UC1(
        img: ARR_8U2D, gamma: float = 1., ratio: float = None, bias: float = 0
    ) -> ARR_8U2D:
    """Intensity transformation by gamma correction:
...
def log_transformation(
        img: NDArray, ratio: Optional[bool] = None, bias: float = 0,
        clip_: Optional[bool] = None, maximum: Optional[bool] = None,
        dtype: np.dtype = np.uint8
    ) -> NDArray:
...
def log_transformation_8UC1(
        img: ARR_8U2D, ratio: float = None, bias: float = 0
    ) -> ARR_8U2D:
    """
...
def arctan_transformation_8UC1(
        img: ARR_8U2D, gamma: float = 0.5, center: float = -1
    ) -> ARR_8U2D:
...
def logistic_correction_8UC1(
        img: ARR_8U2D, sigma: float = 7, center: float = -1
    ) -> ARR_8U2D:
    """
...
@wraparound(False)
def beta_correction(
        img: NDArray, a: float = -1, b: float = -1
    ) -> NDArray:
    ...
    if img.dtype == np.uint8:
        output = betainc(a, b,
                         np.multiply(img, (1/255), dtype=np.float32)) * 255
    else:
...
@boundscheck(False)
@wraparound(False)
def beta_correction_8UC1(
        img: ARR_8U2D, a: float = 2, b: float = 2
    ) -> ARR_8U2D:
    """
...
def auto_gamma_correction_PB_PZ(img: NDArray):
    ...
    if img.dtype == np.uint8:
        gamma = np.divide(
            np.log10(0.5),
            np.log10(np.divide(np.mean(img, axis=(0,1)), 255)))
        if img.ndim == 2:
        ...
        gamma = np.divide(
            np.log10(0.5),
            np.log10(np.mean(img, axis=(0,1))))
        if img.ndim == 2:
            return gamma_correction(img, gamma)
        else:
            output = np.empty_like(img)
            for i in range(img.shape[2]):
                output[:,:,i] = gamma_correction(img[:,:,i], gamma[i],
                                                 dtype=img.dtype)
            return output
...
@njit(
    ...
    nogil=True, cache=True, fastmath=True,)
@wraparound(False)
def histogram_equalization(img: ARR_8U2D) -> ARR_8U2D:
...
def __histogram_matching(he_img, pdf):
    ...
    return commons.transform(he_img, G_inverse)
@boundscheck(False)
@wraparound(False)
def histogram_matching(img: ARR_8U2D, pdf: np.ndarray):
