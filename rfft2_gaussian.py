__all__ = [
    'get_freq_gaussian_kernel', 'get_freq_gaussian_kernel_pos'
    'gaussian_lowpass', 'gaussian_highpass', 'gaussian_bandpass',
    'gaussian_bandreject', 'gaussian_notch_reject',
]
...
def gaussian_lowpass(
    ...
    if parallel:
        return commons.nb_outer(kernel_y, kernel_x)
    else:
...
def gaussian_highpass(
    ...
    kernel_x = get_freq_gaussian_kernel_pos(size[1], sigma_x)
    if parallel:
        return commons.outer_bias_parallel(kernel_y, kernel_x, 1.)
    else:
        return commons.outer_bias(kernel_y, kernel_x, 1)
...
def gaussian_bandreject_parallel(
    ...
    """
    return np.subtract(
            1, gaussian_bandpass_parallel(size, band_center, band_width))
...
def gaussian_pair_notch_reject(
    ...
    """
    kernel1 = gaussian_single_notch_reject(
        size, sigma, center)
    kernel2 = gaussian_single_notch_reject(
        size, sigma, (-center[0], -center[1]))
...
def gaussian_notch_reject(
    ...
    """
    is_iters = [
        is_iterable(cutoff), is_iterable(centers[0])
    ]
    if np.any(is_iters):
        ...
        for i in range(n):
            output = np.multiply(
                output,
                gaussian_pair_notch_reject(size, cutoff[i], centers[0])
            )
    else:
