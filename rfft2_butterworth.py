__all__ = [
    ...
    'butterworth_bandreject', 'butterworth_notch_reject'
]
...
from commons import is_iterable
from img_type import ARR_32F2D, KER_SIZE
...
@njit(
    __SIGNATURE_BUTTERWORTH_LOWPASS,
    nogil=True, cache=True, fastmath=True, error_model="numpy")
@wraparound(False)
...
@njit(
    __SIGNATURE_BUTTERWORTH_LOWPASS,
    nogil=True, cache=True, fastmath=True, error_model="numpy", parallel=True)
@wraparound(False)
...
def butterworth_bandpass(
        size: KER_SIZE, band_center: float, band_width: float,
        order: float = 1, parallel: bool = False
    ) -> ARR_32F2D:
    """Return the Butterworth bandpass filter with order `n`.
                                       (D(u,v)*W)**(2n)
        filter(u,v) = -----------------------------------------------,
    ...
    if (band_center < 0 or band_width <= 0 or order <= 0):
        ...
        return
    if parallel:
        return butterworth_bandpass_parallel(
            size, band_center, band_width, order)
    else:
        return butterworth_bandpass_nonparallel(
            size, band_center, band_width, order)
...
def butterworth_bandreject(
    ...
    if (band_center < 0 or band_width <= 0 or order <= 0):
        ...
        return
    if parallel:
        return butterworth_bandreject_parallel(size, band_center,
                                               band_width, order)
    else:
        return butterworth_bandreject_nonparallel(size, band_center,
                                                  band_width, order)
...
def butterworth_pair_notch_reject(
    ...
    kernel2 = butterworth_single_notch_reject(size, cutoff, order,
                                              (-center[0], -center[1]))
...
def butterworth_notch_reject(
        size: KER_SIZE, cutoff: Union[float,Tuple[float]],
    ...
    """
    is_iters = [
        is_iterable(cutoff), is_iterable(orders), is_iterable(centers[0])
    ]
    if np.any(is_iters):
        ...
        # The first pair.
        output = butterworth_pair_notch_reject(size, cutoff[0], orders[0],
                                               centers[0])
        # The others.
        for i in range(1, n):
            output = np.multiply(
                output,
                butterworth_pair_notch_reject(size, cutoff[i], orders[i],
                                              centers[i])
            )
    else:
