...
from img_type import IMG_COLOR, IMG_ARRAY, IMG_GRAY, IMG_8U, ARR_8U3D
...
def color_slicing_by_dist(
        img: IMG_COLOR, dist: IMG_GRAY, radius: Union[float, int],
    ...
    for y in range(size[0]):
        for x in range(size[1]):
            if dist[y,x] > radius:
                ...
                setitem(getitem(output, y), x,
                        getitem(getitem(img, y), x))
    return output
...
def color_dist_2norm(img: IMG_COLOR, center: type_center) -> IMG_COLOR:
    ...
    dist = cv2.add(
        cv2.pow(cv2.subtract(img[:,:,2], center[2], dtype=cv2.CV_32S), 2),
        dist)
    return dist
...
def color_dist_1norm(img: IMG_COLOR, center: type_center) -> IMG_COLOR:
    dist = cv2.add(
        ...
        dtype=cv2.CV_32S)
    dist = cv2.add(
        cv2.absdiff(img[:,:,2], center[2]), dist,
        dtype=cv2.CV_32S)
    return dist
...
@wraparound(False)
def color_slicing(
        img: IMG_COLOR, center: type_center, radius: Union[float, int],
        bg: Union[float, int] = 127, norm: Literal[0,1,2] = 2
    ) -> IMG_COLOR:
    if not norm:
...
def rgba_to_rgb(
    ...
    if bg == 'w': # white
        ...
        for i in range(3):
            output[:,:,i] = np.add(
                np.multiply(img[:,:,i], alpha, dtype=np.float32),
                b, dtype=np.float32)
    elif bg == "k": # black
        for i in range(3):
...
def yiq_to_rgb(img: IMG_ARRAY) -> IMG_ARRAY:
    if img.ndim==2:
        return
    output = np.empty_like(img, dtype=np.float32)
    weight = np.array([[ 1,  0.95569,  0.61986],
                       [ 1, -0.27158, -0.64687],
                       [ 1, -1.10818,  1.70506]], np.float32)
    for i in range(3):
        output[:,:,i] = np.dot(img, weight[i])
    return output
...
def yuv_to_rgb(img: IMG_ARRAY) -> IMG_ARRAY:
    if img.ndim==2:
        return
    output = np.empty_like(img, dtype=np.float32)
    weight = np.array([[ 1,  0.00093,  1.40169],
                       [ 1, -0.3437,  -0.71417],
                       [ 1,  1.77216,  0.00099]], np.float32)
    for i in range(3):
        output[:,:,i] = np.dot(img, weight[i])
    output[:,:,0] -= 179.53536
    output[:,:,1] += 135.40736
...
def xyz_to_rgb(img: IMG_ARRAY) -> IMG_ARRAY: #CIE 1931 XYZ to RGB
    if img.ndim==2:
        return
    output = np.empty_like(img, dtype=np.float32)
    weight = np.array([[ 3.2404542, -1.5371385, -0.4985314],
...
def rgb_to_hsv(img: IMG_ARRAY) -> IMG_ARRAY:
    ...
    #Calculate H
    n = cMax==img[:,:,0] # cMax = R
    output[n,0] = np.mod(np.divide(img[n,1]-img[n,2],delta[n], where=delta[n]!=0), 6)
...
def hsv_to_rgb(img: IMG_ARRAY) -> IMG_ARRAY:
    ...
    output[index,0], output[index,1], output[index,2] = (
        img[index,2], t[index], q[index]
    )

    index = i==1
    output[index,0], output[index,1], output[index,2] = (
        q[index], img[index,2], p[index]
    )

    index = i==2
    output[index,0], output[index,1], output[index,2] = (
        p[index], img[index,2], t[index]
    )

    index = i==3
    output[index,0], output[index,1], output[index,2] = p[index], q[index], img[index,2]

    index = i==4
    output[index,0], output[index,1], output[index,2] = t[index], p[index], img[index,2]

    index = i==5
    output[index,0], output[index,1], output[index,2] = img[index,2], p[index], q[index]
...
def rgb_to_hsl(img: IMG_ARRAY) -> IMG_ARRAY:
    ...
    output[index,0] = np.mod(np.divide(img[index,1]-img[index,2],delta[index], where=delta[index]!=0), 6)

    index = cMax==img[:,:,1] # cMax = G
    output[index,0] = np.divide(img[index,2]-img[index,0],delta[index], where=delta[index]!=0) + 2

    index = cMax==img[:,:,2] # cMax = B
    output[index,0] = np.divide(img[index,0]-img[index,1],delta[index], where=delta[index]!=0) + 4
...
def hsl_to_rgb(img: IMG_ARRAY) -> IMG_ARRAY:
    ...
    X = C * (1 - np.abs( np.mod( img[:,:,0]/60, 2) - 1)) #C × (1 - |(H / 60°) mod 2 - 1|)
    # m = img[:,:,2] - C/2 # L - C / 2
