__all__ = [
    'threshold_tovalue',
    'threshold_tovalue_inv',
    'threshold_binary',
    'threshold_binary_inv',
    'thresholding',
    'threshold_by_mask',
    'threshold_by_mask_inv',
    'get_threshold_mean',
    'get_threshold_itermean',
    'get_threshold_otsu',
    'get_threshold_max_entropy',
    'get_threshold_yen',
    'get_threshold_moments',
    'get_auto_thresholding_names',
    'auto_threshold',
    'auto_threshold_overall',
    'kmeans_seg_8UC1',
    'kmeans_seg_8UC3',
    'kmeans_seg',
    'superpixel_seg',
...
from src.utils.img_type import Arr8U2D, Arr8U3D, IMG_ARRAY, IMG_GRAY
...
def thresholding(
    ...
    if threshold_type == 'binary' and not inv:
        ...
        raise ValueError('thresholdTypes should be either "binary" or "tovalue".')
...
def get_threshold_yen(img: Arr8U2D) -> np.float32:
    ...
    for i in range(256):
        ...
        if 0 < prod < 1:  # implies 0 < cdf < 1.
            pre = -np.log(prod) + 2 * np.log(cdf * (1 - cdf))
            if pre > maxi:
                maxi = pre
                index = i
    return index  # type: ignore
...
def get_threshold_moments(img: Arr8U2D) -> np.float32:
    ...
    x1 = (raw_moments[0] * raw_moments[2] - raw_moments[1] ** 2) / (
        raw_moments[1] - raw_moments[0] ** 2
    )
    x2 = (raw_moments[0] * raw_moments[1] - raw_moments[2]) / (
        raw_moments[1] - raw_moments[0] ** 2
    )
    x0 = 0.5 - (raw_moments[0] + x2 / 2) / np.sqrt(x2**2 - 4 * x1)
    index = 0
    mini = np.inf
...
def get_auto_thresholding_names():
    ...
    """
    return ('mean', 'itermean', 'otsu', 'max_entropy', 'yen', 'moments')


def auto_threshold(img: Arr8U2D, auto_type: str) -> np.float32:
...
def auto_threshold_overall(img: Arr8U2D):
    ...
    return [(threshold_binary(img, val), val) for val in values]
...
def kmeans_seg_8UC1(
    ...
    return helpers.transform(img, best_clusters)
...
def superpixel_slic(
    ...
    model = createSuperpixelSLIC(img, algorithm, region_size=region_size, ruler=ruler)
    model.iterate(iter_)
    if not boundary:
...
def superpixel_lsc(
    ...
    model = createSuperpixelLSC(img, region_size=region_size, ratio=ratio)
    model.iterate(iter_)
    if not boundary:
        label = model.getLabels()
        output = np.empty_like(img, dtype=np.float32)
...
def superpixel_seeds(
    ...
    shape = img.shape
    if img.ndim == 2:
        model = createSuperpixelSEEDS(*shape[:2], 1, n_segments, num_levels)
    model.iterate(iter_)
    if not boundary:
        label = model.getLabels()
        output = np.empty_like(img, dtype=np.float32)
...
def grabcut(
    ...
    rect=None,
    iter_: int = 10,
    mode: Literal[0, 1, 2, 3] = GC_INIT_WITH_MASK,
):
    shape = img.shape
    if mask is None:
