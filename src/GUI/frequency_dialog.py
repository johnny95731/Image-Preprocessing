...
from src.GUI.interface import BasicDialog
...
class BasicFilterDialog(BasicDialog):
    ...
    # Functions
    def main_work(self):
        import src.filters.rfft2_filters as rfft2_filters

        # Get Args
        type_ = self.combo_box['type'].currentIndex()
        filter_ = self.combo_box['arg'].currentIndex()
        args = [np.float32(self.spin_box[f'arg{j}'].value()) for j in range(3)]
        # Filtering
        if not type_:  # Gaussian
            if not filter_:
                mask = rfft2_filters.gaussian_lowpass(self.rfft_size, args[0], args[1])
            elif filter_ == 1:
                mask = rfft2_filters.gaussian_highpass(self.rfft_size, args[0], args[1])
            elif filter_ == 2:
                mask = rfft2_filters.gaussian_bandpass(self.rfft_size, args[0], args[1])
            elif filter_ == 3:
