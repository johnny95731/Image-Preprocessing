...
import src.GUI.spatial_dialog as spatial_dialog
import src.GUI.color_dialog as color_dialog
from src.GUI.image_object import Image, BasicOperators
from src.utils.img_type import Arr8U2D, IMG_8U
...
class Mainwindow(QtWidgets.QMainWindow):
    ...
    def __create_main_menubar(self):
        ...
        events = [
            {
                'File': [
                    {
                        'Action': 'New',
                        'triggered': self.__create_new_image,
                        'shortcut': 'Ctrl+N',
            ...
            },
            {
                'Edit': [
                    {
                        'Action': 'Undo',
                        'triggered': self.__image_undo,
                        'shortcut': 'Ctrl+Z',
                        'toolTip': 'Back to last step of this image.',
                    },
                    {
                        'Action': 'Redo',
                        'triggered': self.__image_redo,
                        'shortcut': 'Ctrl+Y',
                        'toolTip': 'Forward to next step of this image.',
                    },
                    {
                        'Action': 'Reset',
                        'triggered': self.__reset_image,
                        'toolTip': '\n'.join((
                    ...
                    {
                        'Action': 'Resize',
                        'triggered': self.__image_resize,
                        'toolTip': '\n'.join(('Resize the image.')),
                    },
                ]
            },
            {
                    ...
                    },
                    {
                        'Intensity Trans.': [
                            ...
                            },
                            {
                                ...
                                'toolTip': 'Intensity transform by piecewise linear function.',
                            },
                        ]
                    },
                    {
            ...
            },
            {
                'Spatial': [
                    {
                        'Action': 'Blur',
                        'triggered': self.__blur,
                    },
                    {
                        'Action': 'Unsharp Mask',
                        'triggered': self.__unsharp_mask,
                    },
                    {
                        'Edge': [
                            {
                                'Action': 'Canny',
                                'triggered': self.__canny,
                            },
                            {
                                'Action': 'Gradiant',
                                'triggered': self.__apply_grad,
                            },
                            # {
                    ...
                    {
                        'Action': 'Morphology',
                        'triggered': self.__basic_morph,
                    },
                    {
                        'Action': 'Noise',
                        'triggered': self.__add_noise,
                    },
                    {
                        'Action': 'Sharpening',
        ...
        def create_nested_menu(parent_menu, childs):
            for child in childs:
                if 'Action' in child and isinstance(child['Action'], str):
                    name = child['Action'].replace(' ', '')
                    act: QtGui.QAction = parent_menu['menu'].addAction(child['Action'])
    ...
    def __create_informations_frame(self):
        ...
        widgets['parent'] = QtWidgets.QDockWidget('Informations', self.__central_widget)
        ...
        self.resizeDocks([widgets['parent']], [230], QtCore.Qt.Orientation.Horizontal)
    ...
    # Basic function
    # -Image view
    def update_all(self, img: IMG_8U | Image | None = None):
        if img is None:
    ...
    def __update_imformation_tree(self, img: IMG_8U | Image):
        if isinstance(img, Image):
            ...
            self.__info_model['size'].setText(f'{img.shape}')
            if img.ndim == 3:
                img: Arr8U2D = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    ...
    @property
    def current_image(self) -> IMG_8U | None:
        return self.__image_view.image

    @property
    def image(self) -> IMG_8U | None:
    ...
    def __move_tab(self, _from: int, _to: int) -> None:
        self.__img_objects[_from], self.__img_objects[_to] = (
            self.__img_objects[_to],
            self.__img_objects[_from],
        )
    ...
    def add_new_tab(
        ...
        if isinstance(img, Image):
            ...
            self.__img_objects.append(Image(img, name, file_extension, path))
        else:
    ...
    def mouse_moved(self, view_pos):
        ...
        if (0 <= row < size[0]) and (0 <= col < size[1]):
            value = img[row, col]
            if img.ndim == 3:
                self.__status_bar.showMessage(f'(x,y)=({col},{row}), (R,G,B)={value}')
            else:
                self.__status_bar.showMessage(f'(x,y)=({col},{row}), Gray={value}')
    ...
    def __create_new_image(self):
        from src.GUI.file_dialog import NewFileDialog
    ...
    def get_paths(cls, parent=None, action: str = 'open'):
        if action == 'open':
            ...
            paths = QtWidgets.QFileDialog.getOpenFileNames(
                parent, 'Open Files', cls.open_directory, filter=filter_
            )[0]
        elif action == 'save':
            formats = ' '.join(Image.support_file_extensions)
            filter_ = f'Images ({formats})'
            paths = QtWidgets.QFileDialog.getSaveFileName(
                parent, 'Save File', cls.save_directory, filter=filter_
            )[0]
        elif action == 'path':
    ...
    def open_file(self) -> None:
        ...
        for path in paths:
            ...
            idx = max(last_slash, last_backslash) + 1
            file_name = self.filename_repeat(path[idx:])
            last_dot = file_name.rindex('.')
            file_extension = file_name[last_dot:]
            file_name = file_name[:last_dot]
            # Read Image
            img = Image.read_image(path)
            self.add_new_tab(img, file_name, file_extension, path=path[:last_slash])
        self.select_last_tab()

    def save_file(self):
        if not self.__img_objects:
            return
        from src.GUI.file_dialog import SaveFileDialog
    ...
    def __read_standard(self, name: str) -> None:
        img = Image.read_image(f'Data/{name}.png')
        self.add_new_tab(img, name, '.png')
        self.select_last_tab()
    ...
    def __image_resize(self):
        if not self.__img_objects:
            return
        from src.GUI.editting_dialog import ResizeDialog
    ...
    def __basic_filter(self):
        if not self.__img_objects:
            return
        from src.GUI.frequency_dialog import BasicFilterDialog
