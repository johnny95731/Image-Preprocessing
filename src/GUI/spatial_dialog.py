...
from src.GUI.image_object import (
    Image,
    SpatialOperators,
...
class BlurDialog(BasicDialog):
    ...
    def __create_args_region(self):
        ...
        for j in range(q):
            ...
            layout_region.addRow(self.widget[f'arg{j}_label'], self.spin_box[f'arg{j}'])
        if self.mode:
    ...
    def __same_ksize(self, checked: bool):
        if checked:
            val = min(self.spin_box['height'].value(), self.spin_box['width'].value())
    ...
    def bluring(self):
        ...
        if self.blur_op in ('median', 'bilateral'):
            ...
            ksize = [
                self.spin_box[direction].value() for direction in ('height', 'width')
            ]
        # Get args
        args = []
        ...
        #
        return SpatialOperators.blurring(self.original_image, self.blur_op, ksize, args)
    ...
    def blur_op_changed(self, blur: str | None = None):
        # Record past value
        ...
        if self.blur_op in ('median', 'bilateral'):
            ...
            info[0] = (self.spin_box['height'].value(), self.spin_box['width'].value())
            for j, _ in enumerate(arg_names):
                info[j + 1] = self.spin_box[f'arg{j}'].value()
...
class GradientDialog(BasicDialog):
    ...
    def __create_args_region(self):
        ...
        self.combo_box['border'].setCurrentIndex(border_list.index('Reflect_101'))
        ...
        layout_region.addRow(self.widget['overflow_label'], self.spin_box['overflow'])
        # -Threshold
        label2 = QtWidgets.QLabel('Threshold', region)
        label2.setFont(self.fonts['title'])
        self.spin_box['threshold'] = QtWidgets.QDoubleSpinBox()
...
class NoiseDialog(BasicDialog):
    ...
    noise_args = {
        ...
        'salt-and-pepper': {
            ...
            'ranges': ((0, 100), (0, 100)),
            'default': [[0.5, 0.5] for _ in range(3)],
            'pdf': None,
        },
        'single color': {
            'arguments': (r'prob(\%)', 'color'),
            'ranges': ((0, 100), (0, 255)),
            'default': [[0.5, 0] for _ in range(3)],
            'pdf': None,
        },
        'beta': {
            'arguments': ('a', 'b', 'max'),
            'ranges': ((0, float('inf')), (-float('inf'), float('inf')), (0, 255)),
    ...
    def confirm(self, e=None):
        if self.__mode == 'new':
            ...
            if not filename:
                ...
                edit_pos = self.widget['name'].mapToGlobal(self.widget['name'].pos())
                edit_size = QPoint(0, self.widget['name'].size().height())
                QToolTip.showText(edit_pos - edit_size, BasicDialog.filename_tooltip())
                return None
            filename = ''.join([filename, self.combo_box['file_format'].currentText()])
        ...
        for i, c in enumerate(self.channels):
            for j in range(3):
                if j < len_arg:
                    ...
                    self.spin_box[f'arg{c}{j}'].setRange(*self.noise_info['ranges'][j])
        ...
        if self.noise != 'none':
            for i, c in enumerate(self.channels):
                for j in range(len(self.noise_info['arguments'])):
                    self.noise_info['default'][i][j] = self.spin_box[
                        f'arg{c}{j}'
                    ].value()
        self.noise = noise
    ...
    def color_changed(self, index: int):
        ...
        self.widget['bg_Red_label'].setText(red)
...
class ThresholdDialog(BasicDialog):
    ...
    def __init__(self, parent: QWidget):
        ...
        if self.is_color:
            self.grayscale = cv2.cvtColor(self.original_image, cv2.COLOR_RGB2GRAY)
    ...
    def __create_args_region(self):
        ...
        auto_list = [
            'None',
            *SegmentationOperators.get_auto_global_thresholding_names(),
        ]
        ...
        self.spin_box['threshold'].editingFinished.connect(self.threashold_edited)
        layout_region.addRow(label2, self.spin_box['threshold'])
        # -Output Grascale
        self.button['grayscale'] = QtWidgets.QCheckBox('Grayscale')
        self.button['grayscale'].clicked.connect(self.update_view)
