...
from src.GUI.interface import BasicDialog, MplCanvas, LabelThread
from src.GUI.image_object import Image, NoiseOperators
...
class NewFileDialog(BasicDialog):
    ...
    def __create_new_region(
        ...
        file_ext = [ext[1:] for ext in Image.support_file_extensions]
...
class SaveFileDialog(BasicDialog):
    ...
    jpeg_flags = {'quality': {'flag': 1, 'range': (0, 100), 'default': 95}}
    jpeg2000_flags = {'compression': {'flag': 272, 'range': (0, 1000), 'default': 1000}}
    png_flags = {'compression': {'flag': 16, 'range': (0, 9), 'default': 1}}
    webp_flags = {'quality': {'flag': 64, 'range': (0, 100), 'default': 95}}
    ...
    def __create_name_region(
        ...
        title.setFont(self.fonts['title'])
        # -Line Edit
        self.widget['name'] = QtWidgets.QLineEdit(self.image_object.name, region)
        ...
        file_ext = [ext[2:] for ext in Image.support_file_extensions]
        self.combo_box['file_format'] = QtWidgets.QComboBox(region)
        self.combo_box['file_format'].addItems(file_ext)
        ...
        layout_region.addRow(self.widget['name'], self.combo_box['file_format'])
        ...
        self.widget['path'].setMinimumWidth(self.widget['name'].size().width())
        # -Open directory
    ...
    def confirm(self):
        ...
        for j, val in enumerate(info.values()):
            options.extend([val['flag'], self.spin_box[f'arg{j}'].value()])
