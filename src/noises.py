__all__ = [
    ...
    'add_gaussian_noise',
    'add_rayleigh_noise',
    'add_rayleigh_noise_8UC1',
    'add_gamma_noise',
    'add_gamma_noise_8UC1',
    'add_exponential_noise',
    'add_exponential_noise_8UC1',
...
def add_gamma_noise(img: IMG_GRAY, shape: float, scale: float) -> Arr32F2D:
    ...
    if scale > 0:
        for y in range(size[0]):
            for x in range(size[1]):
                output[y, x] = np.add(img[y, x], random.gammavariate(shape, scale))
    elif scale < 0:
        scale = -scale
        for y in range(size[0]):
            for x in range(size[1]):
                output[y, x] = np.subtract(img[y, x], random.gammavariate(shape, scale))
    return output
...
def add_beta_noise(img: Arr8U2D, a: float, b: float, maximum: float = 255) -> Arr8U2D:
    ...
    if b > 0:
        for y in range(size[0]):
            for x in range(size[1]):
                output[y, x] = np.add(img[y, x], maximum * random.betavariate(a, b))
    elif b < 0:
        b = -b
        for y in range(size[0]):
            for x in range(size[1]):
                output[y, x] = np.subtract(
                    img[y, x], maximum * random.betavariate(a, b)
                )
    return output
