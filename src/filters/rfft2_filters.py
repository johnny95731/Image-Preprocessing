...
from src.utils.helpers import outer_bias_parallel
...
def linear_motion_blur(
    ...
    if size[0] % 2:
        ...
        output[quant] = np.multiply(
            np.divide(T, dist), np.multiply(np.sin(dist), np.exp(-1j * dist))
        )
...
def bandpass_filter(
    ...
    **kwargs : dict[float]
    ...
    if name == 'gaussian':
        ...
        order = kwargs.get('order', 1)
        bandpass = butterworth_bandpass(size, band_center, band_width, order, parallel)
    return bandpass
...
def geometric_mean_filter(
    ...
    winner = np.power(np.divide(mask, np.power(mask, 2) + k), np.subtract(1, alpha))
