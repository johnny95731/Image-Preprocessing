__all__ = [
    "DCT1D", "IDCT1D", "DCT2D", "haar", "haar_wavelet", "haar_same",
    "haar_wavelet_same", "steganography_decomposition",
    "steganography_reconstruction", "randon_transform",
    "inv_randon_transform_standard", "window_kernel", "window_kernel_half",
...
from img_type import IMG_GRAY, ARR_8U2D, ARR_32F2D, ARR_64F2D
...
def DCT1D(vector): # 1-D Discrete Cosine Transform
    ...
    if n == 1:
        ...
        beta  = [(vector[i] - vector[-(i + 1)]) / (cos((i + 0.5) * pi / n) * 2.0)
            for i in range(half)]
        alpha = DCT1D(alpha)
        beta  = DCT1D(beta)
        result = []
...
@boundscheck(False)
@wraparound(False)
def haar_wavelet_same(img: IMG_GRAY, level: int = 1) -> List[ARR_32F2D]:
    """Return the Haar wavelet transform of an image.
...
def steganography_decomposition(
        img: ARR_8U2D, return_bool: bool = False
    ) -> List[ARR_8U2D]:
...
def steganography_reconstruction(
        imgs: List[ARR_8U2D], return_bool: bool = False
    ) -> ARR_8U2D:
    ...
    if return_bool:
        return np.sum(
            [np.multiply(img, 2**n, dtype=np.uint8)
...
@wraparound(False)
def randon_transform(img: IMG_GRAY, deg: float = .1) -> ARR_64F2D:
    ...
    for ind,j in enumerate(range(0,size[1]),quant_rho-half_y):#range(quantRho-halfY,quantRho+size[1]-halfX) # rho
        val = np.sum(img[:,j])
        if val>maxi:
            maxi = val
        output[0,ind] = val
...
def inv_randon_transform_standard(img: IMG_GRAY, size):
    ...
    for k in range(quantDeg):
        for y in range(size[0]):
            y_ = np.multiply(halfX,matCos[k]) + np.multiply(y-halfY,matSin[k]) - quantRho
            rho = -y_
            for x in range(size[1]):
